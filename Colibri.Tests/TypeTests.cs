...
public class TypeTests
{
    ...
    [InlineData("(real? 3+0i)", true)]
    [InlineData("(real? 3.5)", true)]
    [InlineData("(real? 8/4)", true)]
    [InlineData("(real? -2.5+0i)", true)]
    [InlineData("(real? -2.5+0.0i)", false, Skip = "Currently unable to tell the difference between 0 and 0.0 with .NET's Complex type")]
    ...
    [InlineData("(rational? +nan.0)", false)]
    [InlineData("(rational? 3)", true)]
    [InlineData("(rational? 3.5)", true)]
    [InlineData("(rational? 6/10)", true)]
    [InlineData("(rational? 6/3)", true)]
    [InlineData("(rational? 3+0i)", true)]
    [InlineData("(rational? 3.5+0i)", true)]
    [InlineData("(rational? 3+1i)", false)]
    [InlineData("(rational? 3+0.0i)", false, Skip = "Currently unable to tell the difference between 0 and 0.0 with .NET's Complex type")]
    [InlineData("(integer? 3+0i)", true)]
    [InlineData("(integer? 3.5+0i)", false)]
    [InlineData("(integer? 3+1i)", false)]
    [InlineData("(integer? 3.0)", true)]
    [InlineData("(integer? 3.5)", false)]
    [InlineData("(integer? 8/4)", true)]
    [Theory]
